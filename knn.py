...
class KNN:
    ...
    def _predict(self, x):
        ...
        if self.distance == "euclidean":
            ...
            distances = [self._euclidean_distance(x, x_train.values if hasattr(x_train, "values") else x_train) for x_train in self.X_train]
        
        elif self.distance == "manhattan":
            distances = [self._manhattan_distance(x, x_train.values if hasattr(x_train, "values") else x_train) for x_train in self.X_train]

        elif self.distance == "minkowski":
            distances = [self._minkowski_distance(x, x_train.values if hasattr(x_train, "values") else x_train, self.p) for x_train in self.X_train]
      
        else:
            raise ValueError("Distance metric not recognized. Use 'euclidean', 'manhattan', or 'minkowski'.")
